...
| `compact-summarizer-lite` | 長文から必要な情報だけを抽出し、短い箇条書きへ強く圧縮 | `$compact-summarizer-lite この会話ログの要点だけまとめて` | 要約する文章 | 原文の事実・不確実性・時系列を保った短い箇条書き |
...
| `story-to-manga` | 怪談・体験談・一次資料・説明文などを、台本から画像生成・吹き出し・写植まで一貫制作 | `$story-to-manga この怪談を8ページの完成漫画にして` | 原文・メモ・根拠のある要約。任意でページ数、形式、画風、人物資料、書体、希望する停止段階 | 適応方針、台本、ネーム、キャラ/背景基準画、写植前画像、写植計画、吹き出し・写植済み完成ページ、QA結果 |
